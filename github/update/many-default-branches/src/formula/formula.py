...
def run(token, username, default_branch, repos):
    ...
    for repository in repositories:
        ...
        if default_branch in branches:
            ...
            input_flag_cmd = f"rit github update repo default-branch --rit_repo_owner=\"{username}\" --rit_github_repo=\"{repository}\" --rit_repo_branch=\"{default_branch}\""
            os.system(f"{input_flag_cmd}")
        else:
            print(f"\n🛠  Creating new \033[36m{default_branch}\033[0m branch for \033[36m{repository}\033[0m...")
            input_flag_cmd = f"rit github create branch --rit_repo_owner=\"{username}\" --rit_github_repo=\"{repository}\" --rit_repo_branch=\"{default_branch}\" --rit_branch_default=\"yes\""
            os.system(f"{input_flag_cmd}")
